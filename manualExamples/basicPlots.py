...
draw.gen_fig_scatter(cl2, show_end=False, show_mean=True).show()

# draw histogram
draw.gen_fig_histo_rel_to_mean(db.create_chunks(idx_start=5)).show()
draw.gen_fig_histo_rel_to_mean(db.create_chunks(idx_start=5), show_start_not_end=True,
                               show_run_not_operation=False).show()